    ...
    return (<div className="flex h-full">
        <div className="min-w-[398px] border-r border-r-gray">
            ...
            <div className="flex flex-col">
                {chats.length > 0 && chats.map((chat: Chat) => <Link
                    to={`/messages/${chat.user1.username === user.username ? chat.user2.username : chat.user1.username}`}
                    key={chat._id} className="flex gap-3 py-2 px-6">
                    <img src={chat.user1.username === user.username ? chat.user2.profile_image : chat.user1.profile_image}
                    alt="profile_image"
                    className="rounded-[50%] object-cover w-14 h-14"/>
                    <div className="flex flex-col gap-1.5">
                        <p className="text-sm">{chat.user1.username === user.username ? chat.user2.username : chat.user1.username}</p>
                        <p className="text-xs text-darkgray">
                            {chat.last_message.author.username === user.username ? 'You' :
                                chat.last_message.author.username} sent a message. · {formatDate(new Date(chat.last_message.createdAt))}</p>
                    </div>
                    </Link>)}
            </div>
        </div>
